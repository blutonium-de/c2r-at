...
async function sendOrderEmailResend(args: {
  to: string
  subject: string
  html: string
}) {
  ...
  if (!r.ok) {
    throw new Error(`Resend send failed: ${j?.message || JSON.stringify(j)}`)
  }
  return j
}
...
export async function POST(req: Request) {
  try {
    ...
    // ✅ E-Mail senden (nachdem in Sanity "paid" steht)
    try {
      ...
      if (to) {
        ...
        await writeClient
          .patch(String(targetId))
          .set({customerEmailSentAt: new Date().toISOString()})
          .commit()
      }
    } catch (mailErr) {
      // Mail darf NICHT den Capture kaputt machen
